...
/**@lends idx.layout.BorderContainer#*/
{
	...
	_setupChild: function(child) {
		...
		if (child.get("splitter") && !child.get("_openInIdxBorderContainer")) {
...
/**@lends idx.layout._ToggleSplitter#*/
{
	...
	postCreate: function() {		
		...
		// setup "_fixedInIdxBorderContainer" attribute
		var fixedAttr = this.child.get("_fixedInIdxBorderContainer");
		if (fixedAttr === "true") {
	...
	_restore: function() {
		...
		if (this.child) {
			this.child.set("_openInIdxBorderContainer", true);
		}
	...
	_collapse: function(startSize) {
		...
		if (this.child) {
			this.child.set("_openInIdxBorderContainer", false);
		}
...
// CHANGE FORM ROCK
// Change fixed and open attributes to more specified name, to void conflict with 
// other widget
dojo_lang.extend(dijit_Widget, {
	_fixedInIdxBorderContainer: false,
	bidiToggle: false,
	snap: false,
	_openInIdxBorderContainer: true
});
